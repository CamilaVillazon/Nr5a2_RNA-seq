...
cts <- as.matrix(read.csv("/home/cvillazon/mRNA_SNAKEPipes_out/featureCounts/counts_male.tsv",
                    sep="\t", row.names=1, header=TRUE))

# Read in metadata
coldata <- read.csv("/home/cvillazon/DEseq2_males/coldata.tsv", row.names=1, sep="\t")
...
write.table(results, file = "ZT0VvsZT12Tx_males.csv", quote = FALSE, sep = ",",
                 dec = ".", row.names = TRUE,
                 col.names = TRUE)

results <- results(dds, contrast = c("condition", "ZT0V", "ZT0Tx"))
write.table(results, file = "ZT0VvsZT0Tx_males.csv", quote = FALSE, sep = ",",
                 dec = ".", row.names = TRUE,
                 col.names = TRUE)

results <- results(dds, contrast = c("condition", "ZT0V", "ZT12V"))
write.table(results, file = "ZT0VvsZT12V_males.csv", quote = FALSE, sep = ",",
                 dec = ".", row.names = TRUE,
                 col.names = TRUE)

results <- results(dds, contrast = c("condition", "ZT0Tx", "ZT12Tx"))
write.table(results, file = "ZT0TxvsZT12Tx_males.csv", quote = FALSE, sep = ",",
                 dec = ".", row.names = TRUE,
                 col.names = TRUE)

results <- results(dds, contrast = c("condition", "ZT12V", "ZT12Tx"))
write.table(results, file = "ZT12VvsZT12Tx_males.csv", quote = FALSE, sep = ",",
                 dec = ".", row.names = TRUE,
                 col.names = TRUE)

results <- results(dds, contrast = c("condition", "ZT12V", "ZT0Tx"))
write.table(results, file = "ZT12VvsZT0Tx_males.csv", quote = FALSE, sep = ",",
                 dec = ".", row.names = TRUE,
                 col.names = TRUE)
...
write.table(assay(rld),
            file = "rlog_norm_counts_males.csv",
            quote = FALSE,
            sep = ",",
            dec = ".",
            row.names = TRUE,
